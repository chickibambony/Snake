...
void Snake::Reset() {
	...
	m_textbox->Clear();
	m_textbox->Add("You have lost. Your score was: " + std::to_string(m_score));
...
void Snake::IncreaseScore() {
	m_score += 10;
	m_textbox->Add("You have ate apple, score: " + std::to_string(m_score));
}
...
void Snake::CheckCollision() {
	...
	for (auto itr = m_snakeBody.begin() + 1; itr != m_snakeBody.end(); ++itr) {
		if (itr->position == head.position) {
			int segment = m_snakeBody.end() - itr;
			Cut(segment);
			break;
		}
	}
...
void Snake::Cut(int l_segment) {
	...
	m_textbox->Add("Remaining lives: " + std::to_string(m_lives));
